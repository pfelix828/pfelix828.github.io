...
[View on GitHub](https://github.com/pfelix828/enrich-eval){.btn .btn-outline-primary} [Live App](https://enrich-eval.vercel.app){.btn .btn-outline-primary}
...
**Finding 4 — Some things can't be known from one transaction.** Recurring-subscription detection is
the one thing here I *don't* report as a headline score, on purpose. Whether a charge is part of a
subscription is a fact about a *series* of transactions, so a system — or an evaluation — that sees
one line at a time can't honestly measure it. The harness keeps recurring as a failure-mode
demonstration instead: a real gym membership with no telltale word gets missed, and a one-off purchase
at a store named "Monthly Market" wrongly trips the flag because of the word *monthly*. (An earlier
draft scored a perfect 1.0 here. That was an artifact of a tiny answer key where the labels and the
detector keyed on the same words — and honestly, *any* number I'd quote would mostly reflect how many
hard cases I chose to include, not real performance.) The real fix isn't a cleverer string rule; it's
...
live demo is a Next.js app that opens on the complaint-to-roadmap investigation — every number it
shows is exported from the repo's report artifacts. The harness is model-agnostic — the same scoring, investigation, and regression layers work whether the
enricher is a simple rules engine or a large language model, which you can swap in with one setting.

## Stack

Python | pandas | scikit-learn | Next.js live demo | Streamlit local app | Anthropic API (optional backend)
